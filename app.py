...
app = Flask(__name__)
app.secret_key = "dragon_core_key"

# Настройка базы данных только для заметок
...
# Список твоих друзей (редактируй прямо здесь)
friends_info = {
...
def home():
    ...
    return render_template('index.html', buttons=friends_list, title="DRAGON SQUAD", is_home=True)
...
def delete_note(note_id):
    ...
    note = Note.query.get_or_404(note_id)
    if password == "1234": # Твой секретный пароль для удаления
