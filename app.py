...
# Модель заметок на стене
class Note(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    content = db.Column(db.String(200), nullable=False)
    author = db.Column(db.String(50), default="Аноним")
    recipient = db.Column(db.String(50)) # Кому написали (cat, ruslan и т.д.)
...
# Твои друзья (Просто меняй текст здесь)
friends_info = {
...
@app.route('/')
def home():
    # Создаем список кнопок для главной
...
def show_page(page_name):
    ...
    if data:
        ...
        # Загружаем заметки именно для этого профиля
...
def delete_note(note_id):
    ...
    if password == "1234": # Тот самый пароль
